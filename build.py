...
/* Earth surface: clipIntersection=true with these → removes only x>0 AND y>0 AND z>0 (1/8) */
const earthClipPlanes=[
  new THREE.Plane(new THREE.Vector3(-1,0,0),0),
  new THREE.Plane(new THREE.Vector3(0,0,-1),0),
  new THREE.Plane(new THREE.Vector3(0,-1,0),0)
];
/* Interior layers: inverted normals + clipIntersection=false → keeps only x>0 AND y>0 AND z>0 (1/8) */
